...
from product.forms import ProductForms,TagsForm
...
def addTags(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            tagObj=form.save(commit=False)
            tagObj.save()
            return HttpResponse("<h3>Product added Successfully!!!</h3> <a href='/accounts/home/'> Go Home</a>")
        messages.info(request, 'Tag not added')
        return redirect('add_tag')

    return render(request,'product/addTag.html',{'form':form})

def viewAllTags(request):
    tagObj = Tags.objects.all()
    
    return render(request,'product/viewalltags.html',{'tags':tagObj})
